...
test_stack_basic_push_pop :: proc(t: ^testing.T) {
	s := stack.make_stack(int, context.allocator)
	defer stack.destroy_stack(&s)
...
test_stack_large_scale_lifo :: proc(t: ^testing.T) {
	...
	testing.expect(t, stack.is_empty(&s), "is_empty() returned false on an empty stack")
}
...
test_stack_interleaved_churn :: proc(t: ^testing.T) {
	...
	testing.expect(t, stack.is_empty(&s), "stack failed to clear properly after interleaved churn")
}
